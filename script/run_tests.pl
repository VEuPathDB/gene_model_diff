...
#----------------Get options---------------------#
my %options;
my $result = GetOptions(\%options, 'config|f=s', 'testFile=s');
...
sub utr_change {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    ok(($vb_gene_id eq 'GB0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'UTR change was indetified correct');
  }
}
...
sub gene_validation_ok {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    ok(
      (
              $vb_gene_id eq '08135d5a-14fe-4f94-a37e-7ee7b4947bf3'
          and $cap_gene_id eq '08135d5a-14fe-4f94-a37e-7ee7b4947bf3'
          and $events eq 'identical'
      ),
      'validation of genes were correct'
    );
  }
}
...
sub gene_validation_3exons_ok {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    ok(
      (
              $vb_gene_id eq '2809cfc5-804b-4ac2-abff-d9bc3d230b2b'
          and $cap_gene_id eq '2809cfc5-804b-4ac2-abff-d9bc3d230b2b'
          and $events eq 'identical'
      ),
      'validation of genes were correct'
    );
...
sub no_change {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    $events      = $row->[2];
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'GB0001' and $cap_gene_id eq 'G0001' and $events eq 'identical'),
      'No change identical genes were indetified correct');
  }
}
...
sub no_change_isoforms {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    $events      = $row->[2];
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'identical'),
      'No change isoform identical genes were indetified correct');
  }
...
sub new_gene {
  ...
  my ($vb_gene_id, $cap_gene_id, $events);
  my @test_values = (
    '',       'G0002', 'new_gene',  'new gene indetified correct',
    'GB0001', 'G0001', 'identical', 'identical genes were indetified correct in new gene'
  );
  my $test_count = 0;
  foreach my $row (@{$events_ref}) {
    ...
    print $vb_gene_id
      ...
      . $cap_gene_id
      . $test_values[$test_count + 1]
      . $events
      . $test_values[$test_count + 2] . "\n";
    ok(
      (
              $vb_gene_id eq $test_values[$test_count]
          and $cap_gene_id eq $test_values[$test_count + 1]
          and $events eq $test_values[$test_count + 2]
      ),
      $test_values[$test_count + 3]
    );
    $test_count = 4;
  }
...
sub added_exon {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    $events      = $row->[2];
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'Added exon change_gene genes were indetified correct');
  }
...
sub remove_exon {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'Lost exon change_gene genes were indetified correct');
  }
}
...
sub alter_exon {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'Alter Exon change_gene genes were indetified correct');
  }
}
...
sub gene_in_intron {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    next if ($events eq 'identical');
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq '' and $cap_gene_id eq 'G0002' and $events eq 'new_gene'),
      'new_gene gene in intron were indetified correct');
  }
}
...
sub iso_form {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'new_isoform were indetified correct');
  }
}
...
sub iso_form_lost {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0001' and $events eq 'change_gene'),
      'lost_isoform were indetified correct');
  }
}
...
sub gene_merge {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    ok(
      (
              ($vb_gene_id eq 'G0002:G0001' or $vb_gene_id eq 'G0001:G0002')
          and $cap_gene_id eq 'G0001'
          and $events eq 'merge_gene'
      ),
      'Gene merge were indetified correct'
    );
  }
}
...
sub gene_split {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    $events      = $row->[2];
    print "$vb_gene_id:$cap_gene_id:$events\n";
    ok(($vb_gene_id eq 'G0001' and $cap_gene_id eq 'G0002:G0001' and $events eq 'split_gene'),
      'split gene were indetified correct');
  }
}
...
sub multi_gene_events {
  ...
  foreach my $row (@{$events_ref}) {
    ...
    if ($cap_gene_id eq 'G0001') {
      ...
      ok(($vb_gene_id eq 'G0002' and $cap_gene_id eq 'G0002' and $events eq 'change_gene'),
        'Alter Exon change_gene genes were indetified correct');
    } elsif ($vb_gene_id eq 'G0003') {
      ok(($vb_gene_id eq 'G0003' and $cap_gene_id eq 'G0004:G0003' and $events eq 'split_gene'),
        'split gene were indetified correct');
    } elsif ($cap_gene_id eq 'G0005') {
      ok(($vb_gene_id eq '' and $cap_gene_id eq 'G0005' and $events eq 'new_gene'),
        'new gene indetified correct');
    } elsif ($cap_gene_id eq 'G0006') {
      ok(
        (
                ($vb_gene_id eq 'G0007:G0006' or $vb_gene_id eq 'G0006:G0007')
            and $cap_gene_id eq 'G0006'
            and $events eq 'merge_gene'
        ),
        'Gene merge were indetified correct'
      );
...
sub gene_cluster {
  ...
  my ($gene_cluster_id, $cap_gene_count, $cap_trans_count, $vb_gene_count, $vb_trans_count) =
    @{$cluster_summary->[0]};
  if ($gene_cluster_id == 1) {
    ok(
      $cap_gene_count == 1
        and $cap_trans_count == 1
        and $vb_gene_count == 1
        and $vb_trans_count == 1,
      "cluster summary ok"
    );
  }
