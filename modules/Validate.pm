...
sub validate_gene {
    ...
    foreach my $mRNA (@RNAs) {
        ...
        my $has_author  = ( exists $mRNA_attb{owner}
              and $validation_approved_email{ $mRNA_attb{owner}->[0] } );
        my $is_left_partial = ( exists $mRNA_attb{'is_fmin_partial'}
              and ( $mRNA_attb{'is_fmin_partial'}->[0] eq 'true' ) );
        my $is_right_partial = ( exists $mRNA_attb{'is_fmax_partial'}
              and ( $mRNA_attb{'is_fmax_partial'}->[0] eq 'true' ) );
        my $is_start_partial = (
        ...
        my $stop_seleno = (
            exists $gene_attb{'Note'}
              and ( $gene_attb{'Note'}->[0] eq
        ...
        eval {
            ...
            if (
                !_check_subfeature(
                    \$validation_text, $mRNA_ID, \%gene_hash,
                    \%mRNA_hash,       'Gene:mRNA'
                )
              )
            {
            ...
            foreach my $exon (@exons) {
                ...
                if (
                    !_check_subfeature(
                        \$validation_text, $exon_ID, \%mRNA_hash,
                        \%exon_hash,       'mRNA:exon'
                    )
                  )
                {
            ...
            my $prot_seq =
              Initialize::get_translation( $CDS_sequence, $mRNA_hash{strand} );
            unless ($prot_seq) {
            ...
            if ( $sequence_check ne 'passed' ) {
                ...
                if ( !$start_flag ) {
                    $mRNA_validation_status +=
                      $validation_error_code{CDS_start};
                }
                ...
                if ( !$internal_stop_count == 0 ) {
                    $mRNA_validation_status +=
                      $validation_error_code{CDS_internal_stop};
                }

            }

            $mRNA->add_tag_value(
                'validation_error_code' => $mRNA_validation_status );
            $mRNA->update();
        };
  ...
  #warn "$mRNA_validation_status : $validation_error_code{CDS_stop} : $NO_STOP";
            ...
            and $gene_validation_status < $mRNA_validation_status )
        {
                ...
                and $INTERNAL_STOP == 2 )
            {
                ...
                (
                    $mRNA_validation_status == (
                        $validation_error_code{CDS_stop} +
                          $validation_error_code{CDS_start}
                    )
                )
                and $NO_ATG == 2
