...
sub resolve_maptype_cluster {
  ...
  foreach my $cluster (@{$clusters}) {
    ...
    my (
      $cap_gene_count,      $cap_transcript_count, $vb_gene_count,
      $vb_transcript_count, $cap_max_error,        $vb_max_error
    ) = @{GeneClusters::get_cluster_summary_by_id($dbh, $cluster_id)->[0]};

    if ($cap_gene_count == 1 and $vb_gene_count == 0) {    #new Gene
      _new_gene($dbh, $cluster_id);
    } elsif (($cap_gene_count == 1 and $vb_gene_count == 1)
      and ($cap_transcript_count == $vb_transcript_count))
    {                                                      #gene change or identical gene
...
sub get_gene_mappings_by_id_and_maptype {
  ...
  my $sql =
    "select $return_id from gene_mappings where $type = \'$id\' and map_type = \'$maptype\';";
  my $array_ref = _submit_sql($dbh, $sql);
  return $array_ref;
}
...
sub get_gene_mappings_by_group {
  ...
  my $sql =
"select $gene_id_A from gene_mappings where map_type = \'$map_type\' group by $gene_id_A having count(distinct $gene_id_B) > 1;";
  my $array_ref = _submit_sql($dbh, $sql);
  return $array_ref;
}
...
sub _exon_change {
  ...
  if (defined $cap_max_error and defined $vb_max_error and $cap_max_error > $vb_max_error) {
    $errorLog->error(
      "Cluster $cluster_id was not processed, because cap gene had more errors than reference.");
    return;
  }
...
sub _complex_split_merge_gene {
  ...
  foreach my $transcript_pair (@{$cluster_transcripts}) {
    ...
    my $cap_gene_id =
      GeneModel::get_distinct_id_by_id($dbh, 'transcript', $cap_trans_id, 'gene', 'cap', 0);
    my $vb_gene_id =
      GeneModel::get_distinct_id_by_id($dbh, 'transcript', $vb_trans_id, 'gene', 'vb', 0);
...
sub get_sql {
  ...
  if ($sql_name eq 'insert_gene_mappings') {
    my $insert_gene_mappings =
      "insert gene_mappings(cap_gene_id,vb_gene_id,map_type) select ?,?,?;";
